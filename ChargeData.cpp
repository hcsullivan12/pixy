...
void pixy_roimux::ChargeData::convertHistos() {
    ...
    for (const auto& eventId : m_eventIds) {
        ...
        *readoutHisto = std::pair<TH2S, TH2S>(
                TH2S(pixelHistoName.c_str(), pixelHistoName.c_str(),
                     m_nSamples, 0, m_nSamples, m_runParams.getNPixels(), 0, m_runParams.getNPixels()),
                TH2S(roiHistoName.c_str(), roiHistoName.c_str(),
                     m_nSamples, 0, m_nSamples, m_runParams.getNRois(), 0, m_runParams.getNRois()));
        ...
        for (unsigned pixel = 0; pixel < m_runParams.getNPixels(); ++pixel) {
            ...
            for (unsigned sample = 0; sample < m_nSamples; ++sample) {
